...
function webpackWrap(argv, callback) {
  ...
  pkg.apm = pkg.apm || {};
  pkg.apm.oldMain = pkg.apm.main;
  pkg.apm.main = '________________';
  ...
  getWebpackOpts({cwd:cwd,debug:true,verbose:argv.verbose,pkg:pkg,fromServer:true}, function(err, webpackOpts) {
    ...
    var testOpts = extend(true, {}, webpackOpts, {
      ...
      module: {
        postLoaders: argv.cov ? [{
          test: /\.js$/,
          exclude: /(test|tests|apm_modules|node_modules)\//,
    ...
    // Get deps to install.
    var query = [];
    if (pkg && pkg.apm && pkg.apm.devDependencies) {
      for (var k in pkg.apm.devDependencies) {
        query.push(k+'@'+pkg.apm.devDependencies[k]);
      }
    }
    ...
    install({
      name: query,
      cwd: cwd,
      registry: pkg && pkg.apm && pkg.apm.registry
    }).then(function() {
...
function getSpecFiles(pkg) {
  var spec = pkg.apm.tests || 'tests/**/*-spec.js';
...
function generateCode(cwd, pkg) {
  var files = require('glob').sync('**/*.*(md|htm|html)', {
    ignore: ['_site/**', 'apm_modules/**', 'node_modules/**'],
    cwd: cwd
  });

  var deps = getDeps(files);
  log.info('deps', deps.join(', '));
  var main = pkg.apm.oldMain || 'index';
  main = main.replace(/^\.\//, '');
...
function generateFile(cwd, pkg) {
  ...
  chokidar.watch('**/*.(md|html)', {
    ignored: /(apm_modules|node_modules|_site)/
...
function copyHtmls() {
  var vfs = require('vinyl-fs');
  var through = require('through2');
  vfs.src(['./**/*.html', '!./_site/**/*.html', '!./apm_modules/**/*.html', '!./node_modules/**/*.html'])
    .pipe(through.obj(function(f, enc, cb) {
