...
exports.getDeps = function(file) {
  ...
  // <script type="text/apm"> .... </script>
  re = /(<script type=\"text\/apm\">)([\s\S]*?)(<\/script>)/gmi;
...
exports.replaceDeps = function(content, file) {
  ...
  // <script type="text/apm"> .... </script>
  re = /(<script type=\"text\/apm\">)([\s\S]*?)(<\/script>)/gi;
  re_match = /(<script type=\"text\/apm\">)([\s\S]*?)(<\/script>)/i;
  ...
  content = content.replace(re, function(replacement) {
    ...
    return match[1].replace('type="text/apm"', '') + code + match[3];
  });
  ...
  if (extname.toLowerCase() === '.md') {
    ...
    content = content.replace(re, function (replacement) {
      ...
      code = transform(code, file);
      var codeMirror =
        '<textarea mode="javascript" class="apm-doc-textarea">' + match[2] + '</textarea>';
    ...
    content = content.replace(re, function (replacement) {
      ...
      var codeMirror =
        '<textarea mode="css" class="apm-doc-textarea">' + match[2] + '</textarea>';
