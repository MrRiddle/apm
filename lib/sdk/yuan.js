...
function Yuan(options) {
  ...
  this.server = options.registry || spmrc.get('registry') || 'http://apm.oa.com';
...
Yuan.prototype.request = function(data, callback) {
  ...
  req.on('complete', function(res) {
    if (res.statusCode === 401) {
      log.error('fail', 'authorization is required. try `apm login`');
...
Yuan.prototype.upload = function(data, callback) {
  if (data.private && this.server === 'http://apm.oa.com') {
